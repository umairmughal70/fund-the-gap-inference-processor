...
class DataLoaderUtil:
    ...
    def initGenerateData(self, day, customers=[]):
        ...
        try:
            ...
            if date.today().day == day:
                ...
                customers_param = ''
                if(customers):
                    # customers_param = ','.join(customers)
                    customers_param = ",".join([str(i) for i in customers])
                trans_df,features_df = self.retrieveDataFromDb(customers_param)
    ...
    def retrieveDataFromDb(self, customerIds):
        ...
        try:
            ...
            print("Customers IDS: ",customerIds)
            ...
            if(customerIds==''):
                ...
                bgtQuery = self.__envConfiguration['db.queryBudgetTransApi'] %customerIds
                trans_list    = sqlInstance.executeQuery(bgtQuery)
            ...
            if(customerIds==''):
                ...
                ftrQuery = self.__envConfiguration['db.queryAIFeaturesApi'] % customerIds
                features_list = sqlInstance.executeQuery(ftrQuery)
            # features_list = sqlInstance.executeQuery(self.__envConfiguration['db.queryAIFeatures'])
    ...
    def get_features(self, trans_df,features_df):
        try:
            ...
            # final_df = final_df[["CustomerID","ProductCode","SegmentID","Date","Credit", "Debit", "Balance","Prev_Mean_Bal","Struggle"]]
            final_df = final_df.groupby(["CustomerID", "ProductCode"]).tail(1)
            final_df["Date"] = final_df["Date"] + pd.DateOffset(months= 1)
            # final_df.to_csv(r"G:\POCs\PFM-AI\temp\final_df_inference.csv", index = False)
            return final_df
