...
class ConfigUtil:
    ...
    configJSON     = {
        ...
        "db.predTable"              : "", # PINO_ACCADAP_PREDICTIONS_TABLE
        ...
        "db.queryAIFeatures"        : "", # PINO_ACCADAP_QUERY_AIFEATURES
        "db.queryAIFeaturesApi"     : "", # PINO_ACCADAP_QUERY_AIFEATURES_API
        "db.queryBudgetTrans"       : "", # PINO_ACCADAP_QUERY_BUDGETTRANS
        "db.queryBudgetTransApi"    : "", # PINO_ACCADAP_QUERY_BUDGETTRANS_API
        ...
        "dir.modelObj"              : "", # PINO_ACCADAP_DEPLOYED_MODEL_OBJECT
        "dir.meta_file"             : "", # PINO_ACCADAP_MODEL_META_FILE
        ...
        "dir.appHost"               : "",  # PINO_APP_HOST
        "dir.appContext"            : "",  # PINO_APP_CTXROOT
        "dir.appPort"               : "",  # PINO_APP_PORT
    }
    @staticmethod
    def getInstance():
    ...
    def __loadEnvironmentVariables(self):
        ...
        self.configJSON['db.predTable']                 = env.str('PINO_ACCADAP_PREDICTIONS_TABLE',         validate=[Length(min=1,max=128, error ='Invalid AI database name. Should not be > 128 chars')])
        self.configJSON['db.aiDbName']                  = env.str('PINO_ACCADAP_AI_DB_NAME',                validate=[Length(min=1,max=128, error ='Invalid Predictions table name. Should not be > 128 chars')])
        ...
        self.configJSON['db.queryBudgetTransApi']       = env.str('PINO_ACCADAP_QUERY_BUDGETTRANS_API',     validate=[Length(min=1,         error ='Invalid budget transactions api query. Should not be < 0 chars')])
        self.configJSON['db.queryAIFeatures']           = env.str('PINO_ACCADAP_QUERY_AIFEATURES',          validate=[Length(min=1,         error ='Invalid budget query. Should not be < 0 chars')])
        self.configJSON['db.queryAIFeaturesApi']        = env.str('PINO_ACCADAP_QUERY_AIFEATURES_API',      validate=[Length(min=1,         error ='Invalid API budget query. Should not be < 0 chars')])
        ...
        self.configJSON['dir.modelObj']                 = env.str('PINO_ACCADAP_DEPLOYED_MODEL_OBJECT',     validate=[Length(min=1,max=128, error ='Invalid model object name. Should not be > 128 chars')])
        self.configJSON['dir.meta_file']                = env.str('PINO_ACCADAP_MODEL_META_FILE',           validate=[Length(min=1,max=128, error ='Invalid model meta data name. Should not be > 128 chars')])
        ...
        self.configJSON['dir.appHost']                  = env.str('PINO_APP_HOST',                          validate=[Length(min=7,max=50,  error ='Invalid App host')])
        self.configJSON['dir.appContext']               = env.str('PINO_APP_CTXROOT',                       validate=[Length(min=2,max=20,  error ='Invalid App context root')])
        self.configJSON['dir.appPort']                  = env.str('PINO_APP_PORT',                          validate=[Length(min=2,max=4,   error ='Invalid App Port')])
